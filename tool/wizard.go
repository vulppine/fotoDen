...
func setupWebsite(loc string, theme string) (WebsiteConfig, *generator.WebConfig) {
	...
	w.Name = cmdio.ReadInputReq("What is the name of your website? (required)")
	w.URL = cmdio.ReadInputReq("What is the URL of your website? (required)")
	...
	src := cmdio.ReadInput("Are you going to remotely host your images? If so, type in the URL now, otherwise leave it blank to automatically use local hosting for all images")
	...
	s.ThumbnailFrom = cmdio.ReadInputReq("What size do you want your thumbnails to be? (required)")
	s.DisplayImageFrom = cmdio.ReadInputReq("What size do you want to display your images as in a fotoDen photo viewer? (required)")
	s.DownloadSizes = cmdio.ReadInputAsArray("What sizes do you want easily downloadable?", ",")
...
func setupConfig() generator.Config {
	...
	config.ImageRootDirectory = cmdio.ReadInput("Where do you want your images to be stored in every folder?")
	if isBlank(config.ImageRootDirectory) {
		config.ImageRootDirectory = generator.DefaultConfig.ImageRootDirectory
	}

	config.ImageSrcDirectory = cmdio.ReadInput("Where are you going to store your source-quality images?")
	...
	imageSizes := cmdio.ReadInputAsArray("What image sizes do you want? Separate by comma, no spaces.", ",")
	if imageSizes[0] != "" {
		...
		for _, val := range imageSizes {
			...
			for c != true {
				...
				imageSize.MaxHeight, _ = cmdio.ReadInputAsInt("Maximium height of image?")
				imageSize.MaxWidth, _ = cmdio.ReadInputAsInt("Maximum width of image?")
				scalePercent, _ := cmdio.ReadInputAsFloat("Image scale percent? [0 - 100%]")
...
func setupWebConfig(source string) *generator.WebConfig {
	...
	config.WebsiteTitle = cmdio.ReadInput("What is the title of your website?")
	config.PhotoURLBase = cmdio.ReadInput("Are you going to be using a remote storage provider for your photos? If so, put in the URL to the folder containing your fotoDen-structured images here.")
	...
	config.ThumbnailFrom = cmdio.ReadInput("What size do you want your thumbnails to be?")
	config.DisplayImageFrom = cmdio.ReadInput("What size do you want to display your images as in a fotoDen photo viewer?")
	config.DownloadSizes = cmdio.ReadInputAsArray("What sizes do you want downlodable?", ",")
...
func generateFolderWizard(directory string) (*generator.Folder, error) {
	...
	folder.Name = cmdio.ReadInput("What is the name of this folder/album?")
	folder.Desc = cmdio.ReadInput("What is the description of this folder/album?")
	folder.Static = cmdio.ReadInputAsBool("Will the folder/album webpages have some dynamic elements static?", "y")
	t := cmdio.ReadInputAsBool("Will the folder have a thumbnail?", "y")
	if t {
		thumb := cmdio.ReadInput("Where is the thumbnail located? (direct path or relative to current working directory)")
		if thumb == "" {
...
func updateFolderWizard(fol *generator.Folder) *generator.Folder {
	fol.Name = cmdio.ReadInput("What is the new name of this folder/album?")
	fol.Desc = cmdio.ReadInput("What is the new description of this folder/album?")
